...
public class ClientSchoolData {
    ...
    private static final String[] HOTBAR = new String[6];
    private static boolean hotbarLoaded = false;

    private static void loadHotbar() {
        if (hotbarLoaded) return;
        var mc = net.minecraft.client.Minecraft.getInstance();
        if (mc.player != null) {
            var data = mc.player.getPersistentData();
            var list = data.getList("sihriya_hotbar", 8);
            for (int i = 0; i < 6 && i < list.size(); i++)
                HOTBAR[i] = list.getString(i);
        }
        hotbarLoaded = true;
    }

    public static String getHotbarSpell(int slot) {
        loadHotbar();
        if (slot < 0 || slot >= 6) return "";
        return HOTBAR[slot];
    }

    public static void setHotbarSpell(int slot, String spellId) {
        if (slot < 0 || slot >= 6) return;
        HOTBAR[slot] = spellId != null ? spellId : "";
    }

    public static void noteHotbarCast(int slot, String spellId) {
        lastRequestedSchool = "";
        lastRequestedSpellId = spellId;
        lastRequestedAtMs = System.currentTimeMillis();
        var spell = SpellRegistry.get(spellId);
        lastRequestedCooldownMs = spell != null ? Math.max(750L, spell.cooldown * 50L) : 1000L;
    }
