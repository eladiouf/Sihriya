...
public class GrimoireScreen extends Screen {
    ...
    private void drawOptionsButton(GuiGraphics g, int mouseX, int mouseY) {
        ...
        // Hotbar button
        int hx = x - 64;
        boolean hHover = mouseX >= hx && mouseX <= hx + 60 && mouseY >= y && mouseY <= y + 16;
        g.fill(hx, y, hx + 60, y + 16, hHover ? 0xFF4A6440 : 0xFF2A4030);
        g.drawString(font, "Hotbar", hx + 30 - font.width("Hotbar") / 2, y + 4, 0xFFD4FF00, false);
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (selectedSpell != null && ClientSchoolData.isSpellLearned(selectedSpell.id)) {
            for (int slot = 0; slot < 6; slot++) {
                if (keyCode == 49 + slot) {
                    ClientSchoolData.setHotbarSpell(slot, selectedSpell.id);
                    tong.sihriya.network.NetworkHandler.CHANNEL.sendToServer(
                        new tong.sihriya.network.HotbarUpdatePacket(slot, selectedSpell.id));
                    return true;
                }
            }
            if (keyCode == 259 || keyCode == 261) {
                for (int slot = 0; slot < 6; slot++) {
                    if (selectedSpell.id.equals(ClientSchoolData.getHotbarSpell(slot))) {
                        ClientSchoolData.setHotbarSpell(slot, "");
                        tong.sihriya.network.NetworkHandler.CHANNEL.sendToServer(
                            new tong.sihriya.network.HotbarUpdatePacket(slot, ""));
                    }
                }
                return true;
            }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int hx = optionX - 64;
        if (mouseX >= hx && mouseX <= hx + 60 && mouseY >= optionY && mouseY <= optionY + 16) {
            SihriyaUiSounds.open();
            minecraft.setScreen(new HotbarScreen());
            return true;
        }
